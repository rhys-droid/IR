classdef printing < handle
    properties
        num_layers = 2;
        fig_handle;
        dobot;   % Robot passed from CreateWorld
    ...
    methods
        function self = printing(dobot)
            self.dobot = dobot;  % Assign the passed robot object
            addpath('../Print Files');
        ...
        function moveRoboticArm(self, vertex_matrix)
            ...
            for layer = 1:num_layers
                ...
                for i = 1:size(points_in_layer, 1) - 1
                    ...
                    try
                        q_traj = jtraj(self.dobot.model.getpos(), q_next, 5);  % Fewer points for faster execution
        ...
        function moveRobotHome(self)
            ...
            try
            home_q = self.dobot.model.ikine(transl(0, 0, self.z_offset), self.dobot.model.getpos(), 'mask', [1 1 1 0 0 0], 'tol', 1e-3, 'ilimit', 1000);
            q_traj_home = jtraj(self.dobot.model.getpos(), home_q, 3);  % Faster trajectory to home
